...
id: tr-science
model: chat
territory: [supabase/migrations/0033_*]
forbidden: [packages/ui/src/index.ts, supabase/migrations/0028_leccion_en_espanol.sql]
context: [contracts/fuentes/science.json, supabase/migrations/0028_leccion_en_espanol.sql]
verify: node scripts/deepseek/validar-traduccion.mjs contracts/fuentes/science.json supabase/migrations/0033_science_es.sql
setup: ninguno
rounds: 5
...
La materia **science** (Ciencias) esta sembrada en produccion
...
Te toca **las 5 lecciones**: 38 bloques, 6517 caracteres.
...
`contracts/fuentes/science.json`, que te doy entero mas abajo. Cada bloque
...
Escribe `supabase/migrations/0033_science_es.sql` y haz que salga verde:

```
node scripts/deepseek/validar-traduccion.mjs contracts/fuentes/science.json supabase/migrations/0033_science_es.sql
```

Ese verificador **lo escribio el supervisor, no tu**, y comprueba por codigo de
salida: cobertura de los 38 bloques uno a uno, que el marcado HTML se
...
- Una escritura que reemplace `content` entero en vez de anadir la clave `es`.
  **Perder el ingles seria peor que no traducir**: el verificador lo caza y
  rechaza el parche.
- UUID literales. Se localiza por `subjects.code = 'science'` + el curso
  global de `year_level = 6` + `lessons.ord` + `lesson_blocks.ord`.
- Traducir a medias y dejar bloques fuera. El verificador cuenta los 38.
