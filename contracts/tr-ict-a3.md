...
id: tr-ict-a3
model: chat
territory: [supabase/migrations/0035_*]
forbidden: [packages/ui/src/index.ts, supabase/migrations/0028_leccion_en_espanol.sql]
context: [contracts/fuentes/ict-a3.json, supabase/migrations/0028_leccion_en_espanol.sql]
verify: node scripts/deepseek/validar-traduccion.mjs contracts/fuentes/ict-a3.json supabase/migrations/0035_ict_es.sql
setup: ninguno
rounds: 5
...
Te toca **la leccion 3**: 7 bloques, 1470 caracteres.
...
`contracts/fuentes/ict-a3.json`, que te doy entero mas abajo. Cada bloque
...
Escribe `supabase/migrations/0035_ict_es.sql` y haz que salga verde:

```
node scripts/deepseek/validar-traduccion.mjs contracts/fuentes/ict-a3.json supabase/migrations/0035_ict_es.sql
```

Ese verificador **lo escribio el supervisor, no tu**, y comprueba por codigo de
salida: cobertura de los 7 bloques uno a uno, que el marcado HTML se
...
- Traducir a medias y dejar bloques fuera. El verificador cuenta los 7.
...
- Copiar el ingles tal cual en un bloque que tiene palabras, SIN DECLARARLO.
  Si un bloque se escribe igual en los dos idiomas —«bit -> byte -> kilobyte»
  son unidades internacionales— es legitimo dejarlo, pero tienes que
  declararlo con una linea `-- IDENTICO leccion:bloque — motivo`. Declararlo
  en falso, sobre un bloque que si cambia, tambien se rechaza.
