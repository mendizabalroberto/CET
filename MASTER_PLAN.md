...
| AD-3 | Identidad | Identidad sintética sobre Supabase Auth. Alumno: colegio + código + PIN → Edge Function valida → sesión Supabase real. Staff: email+password (MFA para admins) | `auth.uid()` es el eje de todas las RLS |
| AD-4 | PIN | 4 dígitos primaria / 6 secundaria, configurable por colegio. Profesor genera, alumno cambia en primer login. Argon2id, rate limit, lockout | — |
